"""
Condition C2 — R2CCP (Regression-to-Classification CP)
"""
...
print("=" * 62)
print("  Condition C2 — R2CCP (Regression-to-Classification CP)")
...
def train_and_eval(y_tr, y_cal, y_te, label="combined"):
    ...
    # ── R2CCP calibration ────────────────────────────────
    # Regression to Classification bins, density-based nonconformity.
    n_bins = 20; score_range = (1.0, 5.0)
    bins_edge = np.linspace(score_range[0], score_range[1], n_bins + 1)
    bin_mids = (bins_edge[:-1] + bins_edge[1:]) / 2.0
    bin_w = bins_edge[1] - bins_edge[0]

    def _interval_to_density(lo, hi):
        mid_v = (lo + hi) / 2.0
        sigma = max((hi - lo) / 4.0, 0.1)
        probs = np.exp(-0.5 * ((bin_mids - mid_v) / sigma) ** 2)
        probs /= (probs.sum() * bin_w + 1e-10)
        return probs

    def _density_at(lo, hi, yv):
        dens = _interval_to_density(lo, hi)
        idx_d = min(np.searchsorted(bins_edge[1:], yv, side='left'), n_bins - 1)
        return float(dens[idx_d])

    m1c, m2c = pred(X_cal)
    nc = np.array([-_density_at(m1c[i], m2c[i], y_cal[i]) for i in range(len(y_cal))])
    n_cal = len(y_cal)
    qlev  = min((n_cal+1)*(1-ALPHA)/n_cal, 1.0)
    q_hat = float(np.quantile(nc, qlev, method="higher"))
    density_thresh = -q_hat

    m1t, m2t = pred(X_te)
    clo_list, chi_list = [], []
    for i in range(len(m1t)):
        dens = _interval_to_density(m1t[i], m2t[i])
        included = dens >= density_thresh
        if not included.any():
            included[np.argmax(dens)] = True
        included_bins = bin_mids[included]
        lo = float(included_bins.min()) - bin_w / 2
        hi = float(included_bins.max()) + bin_w / 2
        clo_list.append(np.clip(lo + R_SHIFT, 1.0, 5.0))
        chi_list.append(np.clip(hi + R_SHIFT, 1.0, 5.0))
    clo = np.array(clo_list)
...
results = {}
if PER_DIMENSION:
    print(f"\n[Train] Per-dimension  (Condition C2 — R2CCP)")
...
print("\n"+"═"*55)
print("  DEMO — Condition C2 — R2CCP (Regression-to-Classification CP)")
print("═"*55)
...
fig1.suptitle(f"Condition C2 — R2CCP (Regression-to-Classification CP)  |  TubeNet",
...
if PER_DIMENSION:
    ...
    fig2.suptitle(f"C2 R2CCP Per-Dimension Intervals (α={ALPHA})",fontsize=12,fontweight="bold")
...
fig3.suptitle("C2 R2CCP Diagnostics",fontsize=12,fontweight="bold")
...
if PER_DIMENSION:
    ...
    fig4.suptitle("C2 R2CCP — Tube Loss Curves per Dimension",fontsize=12,fontweight="bold")
...
save_json(os.path.join(OUT_DIR,"results_C2.json"),{
    k:{"coverage":round(r["cov_c"],4),"avg_width":round(r["wid_c"],4),
       ...
       "q_hat":round(r["q_hat"],4),"condition":"C2_R2CCP",
