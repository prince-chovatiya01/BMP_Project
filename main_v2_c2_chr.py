"""
Condition C2 — CHR (Conditional Histogram Regression)
"""
...
print("=" * 62)
print("  Condition C2 — CHR (Conditional Histogram Regression)")
...
def train_and_eval(y_tr, y_cal, y_te, label="combined"):
    ...
    # ── CHR calibration ──────────────────────────────────
    # Histogram-based shortest interval estimation.
    n_bins = 10; score_range = (1.0, 5.0)
    bins_edge = np.linspace(score_range[0], score_range[1], n_bins + 1)
    bin_mids = (bins_edge[:-1] + bins_edge[1:]) / 2.0

    def _interval_to_probs(lo, hi):
        covered = (bin_mids >= lo) & (bin_mids <= hi)
        if covered.sum() == 0:
            idx_c = np.argmin(np.abs(bin_mids - (lo + hi) / 2))
            covered[idx_c] = True
        probs = covered.astype(float)
        return probs / probs.sum()

    def _nc_score(lo, hi, yv):
        probs = _interval_to_probs(lo, hi)
        y_bin = min(np.searchsorted(bins_edge[1:], yv, side='left'), n_bins - 1)
        order = np.argsort(-probs)
        rank = np.where(order == y_bin)[0][0]
        return rank / n_bins

    m1c, m2c = pred(X_cal)
    nc = np.array([_nc_score(m1c[i], m2c[i], y_cal[i]) for i in range(len(y_cal))])
    n_cal = len(y_cal)
    ...
    for i in range(len(m1t)):
        lo, hi = m1t[i], m2t[i]
        for expand in np.arange(0.0, 4.0, 0.1):
            lo_e = max(lo - expand, score_range[0])
            hi_e = min(hi + expand, score_range[1])
            if _nc_score(lo_e, hi_e, (lo_e + hi_e) / 2) <= q_hat:
                lo, hi = lo_e, hi_e
                break
...
results = {}
if PER_DIMENSION:
    print(f"\n[Train] Per-dimension  (Condition C2 — CHR)")
...
print("\n"+"═"*55)
print("  DEMO — Condition C2 — CHR (Conditional Histogram Regression)")
print("═"*55)
...
fig1.suptitle(f"Condition C2 — CHR (Conditional Histogram Regression)  |  TubeNet",
...
if PER_DIMENSION:
    ...
    fig2.suptitle(f"C2 CHR Per-Dimension Intervals (α={ALPHA})",fontsize=12,fontweight="bold")
...
fig3.suptitle("C2 CHR Diagnostics",fontsize=12,fontweight="bold")
...
if PER_DIMENSION:
    ...
    fig4.suptitle("C2 CHR — Tube Loss Curves per Dimension",fontsize=12,fontweight="bold")
...
save_json(os.path.join(OUT_DIR,"results_C2.json"),{
    k:{"coverage":round(r["cov_c"],4),"avg_width":round(r["wid_c"],4),
       ...
       "q_hat":round(r["q_hat"],4),"condition":"C2_CHR",
