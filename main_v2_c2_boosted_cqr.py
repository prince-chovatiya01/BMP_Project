...
OUT_DIR       = "bmp_results_C2_BoostedCQR"
...
print("=" * 62)
print("  Condition C2 — Boosted CQR")
...
def train_and_eval(y_tr, y_cal, y_te, label="combined"):
    ...
    # ── Boosted CQR calibration ──────────────────────────
    # Post-hoc gradient boosting correction on top of CQR residuals.
    ...
    clo = np.clip(m1t - q_eff + R_SHIFT, 1.0, 5.0)
    chi = np.clip(m2t + q_eff + R_SHIFT, 1.0, 5.0)
    mid = (clo + chi) / 2.0
...
results = {}
if PER_DIMENSION:
    print(f"\n[Train] Per-dimension  (Condition C2 — Boosted CQR)")
...
print("\n"+"═"*55)
print("  DEMO — Condition C2 — Boosted CQR")
print("═"*55)
...
fig1.suptitle(f"Condition C2 — Boosted CQR  |  TubeNet",
...
if PER_DIMENSION:
    ...
    fig2.suptitle(f"C2 Boosted CQR Per-Dimension Intervals (α={ALPHA})",fontsize=12,fontweight="bold")
...
fig3.suptitle("C2 Boosted CQR Diagnostics",fontsize=12,fontweight="bold")
...
if PER_DIMENSION:
    ...
    fig4.suptitle("C2 Boosted CQR — Tube Loss Curves per Dimension",fontsize=12,fontweight="bold")
...
save_json(os.path.join(OUT_DIR,"results_C2.json"),{
    k:{"coverage":round(r["cov_c"],4),"avg_width":round(r["wid_c"],4),
       ...
       "q_hat":round(r["q_hat"],4),"condition":"C2_BoostedCQR",
