"""
Condition C2 — CQR (Conformalized Quantile Regression)
Trains two quantile regressors (α/2, 1-α/2), calibrates symmetric residual.
"""
...
print("=" * 62)
print("  Condition C2 — CQR (Conformalized Quantile Regression)")
...
def train_and_eval(y_tr, y_cal, y_te, label="combined"):
    ...
    # ── CQR calibration ──────────────────────────────────
    # TubeNet lo/hi ARE quantile estimates (α/2, 1-α/2).
    # nc = max(q_lo(x)-y, y-q_hi(x)) — symmetric residual.
    m1c, m2c = pred(X_cal)
...
results = {}
if PER_DIMENSION:
    print(f"\n[Train] Per-dimension  (Condition C2 — CQR)")
...
print("\n"+"═"*55)
print("  DEMO — Condition C2 — CQR (Conformalized Quantile Regression)")
print("═"*55)
...
fig1.suptitle(f"Condition C2 — CQR (Conformalized Quantile Regression)  |  TubeNet",
...
if PER_DIMENSION:
    ...
    fig2.suptitle(f"C2 CQR Per-Dimension Intervals (α={ALPHA})",fontsize=12,fontweight="bold")
...
fig3.suptitle("C2 CQR Diagnostics",fontsize=12,fontweight="bold")
...
if PER_DIMENSION:
    ...
    fig4.suptitle("C2 CQR — Tube Loss Curves per Dimension",fontsize=12,fontweight="bold")
...
save_json(os.path.join(OUT_DIR,"results_C2.json"),{
    k:{"coverage":round(r["cov_c"],4),"avg_width":round(r["wid_c"],4),
       ...
       "q_hat":round(r["q_hat"],4),        "condition":"C2_CQR",
