"""
Condition C2 — LVD (Locally Valid & Discriminative)
"""
...
print("=" * 62)
print("  Condition C2 — LVD (Locally Valid & Discriminative)")
...
def train_and_eval(y_tr, y_cal, y_te, label="combined"):
    ...
    # ── LVD calibration ──────────────────────────────────
    # Locally-weighted quantile — closer calibration points get more weight.
    bandwidth = 1.0
    m1c, m2c = pred(X_cal)
    nc = np.maximum(m1c - y_cal, y_cal - m2c)
    X_cal_proxy = np.stack([m1c, m2c], axis=1)

    m1t, m2t = pred(X_te)
    X_te_proxy = np.stack([m1t, m2t], axis=1)
    n_cal = len(y_cal)
    clo_list, chi_list, qhat_list = [], [], []
    for i in range(len(m1t)):
        diffs = X_cal_proxy - X_te_proxy[i]
        dists = np.sum(diffs ** 2, axis=1)
        weights = np.exp(-dists / (2 * bandwidth ** 2))
        weights = weights / (weights.sum() + 1e-10)
        order = np.argsort(nc)
        nc_sorted = nc[order]; w_sorted = weights[order]
        cumw = np.cumsum(w_sorted)
        qlev = min((n_cal+1)*(1-ALPHA)/n_cal, 1.0)
        idx = min(np.searchsorted(cumw, qlev * cumw[-1]), n_cal - 1)
        q_i = max(float(nc_sorted[idx]), 0.0)
        clo_list.append(np.clip(m1t[i] - q_i + R_SHIFT, 1.0, 5.0))
        chi_list.append(np.clip(m2t[i] + q_i + R_SHIFT, 1.0, 5.0))
        qhat_list.append(q_i)
    clo = np.array(clo_list)
    chi = np.array(chi_list)
    q_hat = float(np.mean(qhat_list))
    mid = (clo + chi) / 2.0
...
results = {}
if PER_DIMENSION:
    print(f"\n[Train] Per-dimension  (Condition C2 — LVD)")
...
print("\n"+"═"*55)
print("  DEMO — Condition C2 — LVD (Locally Valid & Discriminative)")
print("═"*55)
...
fig1.suptitle(f"Condition C2 — LVD (Locally Valid & Discriminative)  |  TubeNet",
...
if PER_DIMENSION:
    ...
    fig2.suptitle(f"C2 LVD Per-Dimension Intervals (α={ALPHA})",fontsize=12,fontweight="bold")
...
fig3.suptitle("C2 LVD Diagnostics",fontsize=12,fontweight="bold")
...
if PER_DIMENSION:
    ...
    fig4.suptitle("C2 LVD — Tube Loss Curves per Dimension",fontsize=12,fontweight="bold")
...
save_json(os.path.join(OUT_DIR,"results_C2.json"),{
    k:{"coverage":round(r["cov_c"],4),"avg_width":round(r["wid_c"],4),
       ...
       "q_hat":round(r["q_hat"],4),"condition":"C2_LVD",
