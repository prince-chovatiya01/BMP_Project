"""
Condition C2 — Asymmetric CQR
Same as CQR but calibrates lower/upper residuals separately → asymmetric interval.
"""
...
OUT_DIR       = "bmp_results_C2_AsymCQR"
...
print("=" * 62)
print("  Condition C2 — Asymmetric CQR")
...
def train_and_eval(y_tr, y_cal, y_te, label="combined"):
    ...
    # ── Asymmetric CQR calibration ───────────────────────
    # Separate lower/upper nonconformity scores.
    m1c, m2c = pred(X_cal)
    s_lo  = m1c - y_cal   # how much lower bound overshoots
    s_hi  = y_cal - m2c   # how much upper bound undershoots
    n_cal = len(y_cal)
    qlev  = min((n_cal+1)*(1-ALPHA)/n_cal, 1.0)
    Q_lo  = float(np.quantile(s_lo, qlev, method="higher"))
    Q_hi  = float(np.quantile(s_hi, qlev, method="higher"))
    q_hat = (Q_lo + Q_hi) / 2.0  # for reporting

    m1t, m2t = pred(X_te)
    clo = np.clip(m1t - Q_lo + R_SHIFT, 1.0, 5.0)
    chi = np.clip(m2t + Q_hi + R_SHIFT, 1.0, 5.0)
    mid = (clo + chi) / 2.0
...
results = {}
if PER_DIMENSION:
    print(f"\n[Train] Per-dimension  (Condition C2 — Asymmetric CQR)")
...
print("\n"+"═"*55)
print("  DEMO — Condition C2 — Asymmetric CQR")
print("═"*55)
...
fig1.suptitle(f"Condition C2 — Asymmetric CQR  |  TubeNet",
...
if PER_DIMENSION:
    ...
    fig2.suptitle(f"C2 Asym CQR Per-Dimension Intervals (α={ALPHA})",fontsize=12,fontweight="bold")
...
fig3.suptitle("C2 Asym CQR Diagnostics",fontsize=12,fontweight="bold")
...
if PER_DIMENSION:
    ...
    fig4.suptitle("C2 Asym CQR — Tube Loss Curves per Dimension",fontsize=12,fontweight="bold")
...
save_json(os.path.join(OUT_DIR,"results_C2.json"),{
    k:{"coverage":round(r["cov_c"],4),"avg_width":round(r["wid_c"],4),
       ...
       "q_hat":round(r["q_hat"],4),"condition":"C2_AsymCQR",
