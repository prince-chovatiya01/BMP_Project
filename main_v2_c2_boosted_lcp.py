...
OUT_DIR       = "bmp_results_C2_BoostedLCP"
...
print("=" * 62)
print("  Condition C2 — Boosted LCP")
...
def train_and_eval(y_tr, y_cal, y_te, label="combined"):
    ...
    # ── Boosted LCP calibration ──────────────────────────
    # Gradient boosting on localized CP (width-normalized scores).
    from sklearn.ensemble import GradientBoostingRegressor
    m1c, m2c = pred(X_cal)
    nc_width = (m2c - m1c)
    nc_miss  = np.maximum(m1c - y_cal, y_cal - m2c)
    nc = nc_miss / (nc_width + 1e-6)  # normalized by predicted width
    X_cal_proxy = np.stack([m1c, m2c], axis=1)

    gb = GradientBoostingRegressor(n_estimators=100, learning_rate=0.05,
                                   max_depth=3, random_state=42)
    gb.fit(X_cal_proxy, nc)
    nc_adj = nc - gb.predict(X_cal_proxy)
    n_cal = len(y_cal)
    qlev  = min((n_cal+1)*(1-ALPHA)/n_cal, 1.0)
    q_hat = max(float(np.quantile(nc_adj, qlev, method="higher")), 0.0)

    m1t, m2t = pred(X_te)
    X_te_proxy = np.stack([m1t, m2t], axis=1)
    g_te   = gb.predict(X_te_proxy)
    q_eff  = np.maximum(q_hat + g_te, 0.0)
    width  = m2t - m1t
    expand = q_eff * (width + 1e-6)
    clo = np.clip(m1t - expand + R_SHIFT, 1.0, 5.0)
    chi = np.clip(m2t + expand + R_SHIFT, 1.0, 5.0)
    mid = (clo + chi) / 2.0
...
results = {}
if PER_DIMENSION:
    print(f"\n[Train] Per-dimension  (Condition C2 — Boosted LCP)")
...
print("\n"+"═"*55)
print("  DEMO — Condition C2 — Boosted LCP")
print("═"*55)
...
fig1.suptitle(f"Condition C2 — Boosted LCP  |  TubeNet",
...
if PER_DIMENSION:
    ...
    fig2.suptitle(f"C2 Boosted LCP Per-Dimension Intervals (α={ALPHA})",fontsize=12,fontweight="bold")
...
fig3.suptitle("C2 Boosted LCP Diagnostics",fontsize=12,fontweight="bold")
...
if PER_DIMENSION:
    ...
    fig4.suptitle("C2 Boosted LCP — Tube Loss Curves per Dimension",fontsize=12,fontweight="bold")
...
save_json(os.path.join(OUT_DIR,"results_C2.json"),{
    k:{"coverage":round(r["cov_c"],4),"avg_width":round(r["wid_c"],4),
       ...
       "q_hat":round(r["q_hat"],4),"condition":"C2_BoostedLCP",
